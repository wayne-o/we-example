...
(function($, window){
  ...
  // --- EVENTS --- //
  we.events = {};
  we.events.on = function weAddEventListener(eventName, callback){
  ...
  we.events.trigger = function weTriggerEvent(eventName, data){
    ...
    if(we.configs.client.log.events) console.debug('triggering event: '+ eventName);
 ...
 * WE default http client side http transporter
 *
 * Defaults to Jquery .ajax function for transport
 */
(function($, we){

  var plugin = {};

  plugin.name = 'weHTTPjQuery';
  plugin.type = 'http';
  ...
  plugin.enable = function(we) {
    ...
    http.version = plugin.version;
    http.type = plugin.name;

    http.get = function httpGet(url, data, successCallback, errorCallback){

      if(!errorCallback){
        errorCallback = function defaultErrorCallback(xhr, type){
          console.error('Error on we.http.get: ',xhr, type);
        };
      }
    ...
    http.post = function httpPost(url, data, successCallback, errorCallback){
    ...
    http['delete'] = function httpDelete(url, data, successCallback, errorCallback){
    ...
    http.put = function httpPut(url, data, successCallback, errorCallback){
 ...
 * WE default client side socket io
 *
 */
  // --- SOCKET IO ----
...
(function($, we){
    ...
    we.io.connected = false;
    // -- Socket IO

    we.io.connect = function connectInSocketIO() {

      // if socket io exists reconnect
      if(we.io.socket){
        return we.io.reconnect();
      }

      we.io.socket = io.connect(we.config.serverUrl);

      we.hooks.trigger("we-io-build-socket",{
        socket: we.io.socket
      });

      we.io.socket.on('connect', function socketConnected() {
        we.io.connected = true;
        we.hooks.trigger("connect");
        we.events.trigger("socketIoConnect");
      });

      we.io.socket.on('disconnect', function () {
        we.hooks.trigger("disconnect");
        we.events.trigger("socketIoDisconnect");
      });

      we.io.socket.on('reconnect', function () {
        we.hooks.trigger("reconnect");
        we.events.trigger("socketIoReconnect");
      });

      we.io.socket.on('reconnecting', function () {
        we.hooks.trigger("reconnecting");
        we.events.trigger("socketIoReconnecting");
      });

      return we.io.socket;
    };

    we.io.reconnect = function reconnectInSocketIO(){
      return we.io.socket.socket.reconnect();
    };

    we.io.disconnect = function disconnectFromSocketIO(){
      we.io.connected = false;
      return we.io.socket.disconnect();
    };

    we.hooks.on("we-bootstrap-end-after-success", function(){
      we.io.connect();
    });

    we.io.getUser = function getUser(userId, callback){
